...
function field(label, input, hint = "", error = "") {
  // `input` : chaîne HTML (<input>, <select>…) ou { id, html }
  const html = typeof input === "string" ? input : input.html;
  const id = typeof input === "object" && input.id ? input.id : null;
...
export function loginPage({ error, email = "" } = {}) {
  return authLayout({
    ...
    <p class="auth-switch muted">Pas encore de compte ? <a class="link" href="/register">Commencer gratuitement</a></p>`,
  });
}
...
export function registerPage({ error = null, values = {} } = {}) {
  ...
  return authLayout({
    ...
    note: "2 minutes suffisent. Aucune carte bancaire requise.",
    content: `
    ...
    <form method="POST" action="/api/register" data-fetch class="form" novalidate>
      <div class="field-2col">
        ${field("Prénom", { ...input("first_name", { value: values.first_name, placeholder: "Aïcha", required: true, autocomplete: "given-name" }), error: e("first_name") })}
        ${field("Nom", { ...input("last_name", { value: values.last_name, placeholder: "Kossou", required: true, autocomplete: "family-name" }), error: e("last_name") })}
      </div>
      ${field("E-mail", { ...input("email", { type: "email", value: values.email, placeholder: "vous@entreprise.com", required: true, autocomplete: "email" }), error: e("email") })}
      ${field("Téléphone", { ...input("phone", { type: "tel", value: values.phone, placeholder: "+228 90 00 00 00", autocomplete: "tel" }), error: e("phone") })}
      ${field("Mot de passe", { ...input("password", { type: "password", placeholder: "8 caractères minimum", required: true, autocomplete: "new-password" }), error: e("password") })}
      ${field("Nom de l'entreprise", { ...input("company", { value: values.company, placeholder: "Ex. : Kossou & Fils SARL", required: true, autocomplete: "organization" }), error: e("company") })}
      <div class="field-2col">
      ...
      <button type="submit" class="btn primary block">Créer mon compte gratuitement</button>
